...
```ts
type Content =
  | { content_type: 'markdown'; content_body: string; title: string; price_cents: number; visibility: 'public' | 'private' }
  | { content_type: 'external_ref'; content_uri: string; external_identifier: string; title: string; price_cents: number; visibility: 'public' | 'private' }
```
...
## 🔴 Design Bug — `GET /v1/seller/config` (and `/v1/merchant/{store_id}/config`) requires authentication, but the response is needed *before* authentication
