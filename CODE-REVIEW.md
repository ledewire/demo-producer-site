...
| -------------------- | ---------------------------------------------------------------------------------------- | ------------------------------------------------------------------------ |
| **P0 — Bug**         | `requireAuth()` used in Route Handlers — produces redirect responses instead of 401 JSON | All API routes except `auth/`                                            |
| **P1 — Performance** | `getSession()` called 3× per page render (requireAuth, createMerchantClient, NavBar)     | `lib/auth.ts`, `lib/ledewire.ts`, `components/NavBar.tsx`                |
| **P1 — Performance** | Unbounded `content.list()` and `sales.list()` with client-side pagination                | `api/content/route.ts`, `dashboard/page.tsx`, `content/ContentTable.tsx` |
| **P2 — Correctness** | `ForbiddenError` not caught in any Route Handler except login                            | All API routes except `api/auth/login/route.ts`                          |
| **P2 — Quality**     | Deprecated `escape`/`unescape` Unicode base64 hack                                       | `content/[id]/edit/EditContentForm.tsx`                                  |
| **P3 — UX**          | `window.confirm()` used for destructive action confirmations                             | `ContentTable.tsx`, `UserList.tsx`                                       |
| **P3 — Resilience**  | Missing `catch` block in `UserList.handleRemove` — network errors propagate unhandled    | `users/UserList.tsx`                                                     |
